...
class Env():
    ...
    def step(self, x0, x1, x2, x3, x4):
        ...
        # 定义reward
        self.reward = -10*((self.y1)**2 + (self.y2)**2 + (self.y3)**2 + (self.y4)**2)
...
class Actor1(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.features(x)
        x = self.Linear_max_pool(x)
        x = self.Linear_action(x.view(x.shape[0],3584))
        x = self.max_action * x
        return x
...
class Critic1(nn.Module):
    def __init__(self, state_dim, action_dim):
        ...
        self.Linear_max_pool2 = torch.nn.MaxPool1d(5, 3)
    ...
    def forward(self, x, u):
        ...
        q1 = self.features(q1)
        q1 = self.Linear_max_pool(q1)
        q1 = self.Linear_action(q1.view(q1.shape[0], 4096))

        q2 = torch.cat((x, u), 1)
        q2 = q2.view(q2.shape[0], 1, q2.shape[1])
        q2 = self.features2(q2)
        q2 = self.Linear_max_pool2(q2)
        q2 = self.Linear_action2(q2.view(q2.shape[0], 4096))
        return q1, q2
