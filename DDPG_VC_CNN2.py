...
class Critic1(nn.Module):
    ...
    def forward(self, state, action):
        ...
        c1 = c1.view(c1.shape[0], c1.shape[2])
        c2 = c2.view(c2.shape[0], c2.shape[2])# torch.Size([1, 1, 98]) -->torch.Size([1, 97])
        c3 = c3.view(c3.shape[0], c3.shape[2])
        c4 = c4.view(c4.shape[0], c4.shape[2])

        c1 = torch.cat((c1, action), 1)#101
        c2 = torch.cat((c2, action), 1)
        c3 = torch.cat((c3, action), 1)
        c4 = torch.cat((c4, action), 1)

        c1 = torch.relu(self.ls1(c1))
        c2 = torch.relu(self.ls1(c2))##torch.Size([1, 101])->[1,100]
        ...
        q1 = torch.cat((c1, c2, c3, c4), 1)  # torch.Size([1, 160])
        q1 = torch.relu(self.conv_Q1_2(q1.view(q1.shape[0],1,160)))# torch.Size([1, 158])
        q1 = torch.relu(self.MaxPool1d_2(q1))# torch.Size([1, 157])
        ...
        c2 = torch.relu(self.ls1_Q2(c2))  ##torch.Size([1, 101])->[1,100]
        ...
        q2 = torch.cat((c1, c2, c3, c4), 1)  # torch.Size([1, 160])
        q2 = torch.relu(self.conv_Q2_2(q2.view(q2.shape[0],1,160)))
