...
parser.add_argument('--episode_length', default=1000, type=int)  # 回合长度
...
df1 = pd.read_excel('control_Z_data410.xlsx', sheet_name='Sheet1', header=None)
ii = 20000
...
class Env():
    def __init__(self):
        ...
        self.D = 0.001
        self.dt = 0.0002
        self.N = 10000  # 基础激励序列长度
        self.base = math.sqrt(2 * self.D * self.dt) * np.random.randn(self.N + 1)
        self.flength = 300  # 参与神经网络训练的acc序列长度选择
        self.ACC = np.zeros(self.flength)
        self.Q = np.zeros((6, 1))
        self.basecount = 0
        self.acc_end = np.zeros(self.N)
        self.BASE = np.zeros(self.N)
        self.acc = 0
    def push(self, data):
    ...
    def step(self, Fc):
        ...
        A3 = - V * self.Q[4] - (eta1 * omega1 * self.Q[2] + omega1 ** 2 * self.Q[0] * (
                1 - alpha * d / math.sqrt((d - self.Q[1]) ** 2 + (self.Q[0]) ** 2)))
        A4 = - (eta2 * omega2 * self.Q[3] + omega2 ** 2 * self.Q[1] + mu * (omega1 ** 2) * (self.Q[1] - d) * (
                1 - alpha * d / math.sqrt((d - self.Q[1]) ** 2 + (self.Q[0]) ** 2)) - g)
        Q_1 = self.Q[0] + self.Q[2] * 0.5 * dt
        Q_2 = self.Q[1] + self.Q[3] * 0.5 * dt
        Q_3 = self.Q[2] + A3 * 0.5 * dt
        Q_4 = self.Q[3] + A4 * 0.5 * dt
        Q_5 = self.Q[4] + self.Q[5] * 0.5 * dt
        Q_6 = self.Q[5] - (eta * omega0 * self.Q[5] + omega0 ** 2 * self.Q[4]) * 0.5 * dt
        A3_ = - V * Q_5 - (
                eta1 * omega1 * Q_3 + omega1 ** 2 * Q_1 * (1 - alpha * d / math.sqrt((d - Q_2) ** 2 + Q_1 ** 2)))
        A4_ = -(eta2 * omega2 * Q_4 + omega2 ** 2 * Q_2 + mu * omega1 ** 2 * (Q_2 - d) * (
                1 - alpha * d / math.sqrt((d - Q_2) ** 2 + Q_1 ** 2)) - g)
        self.Q[0,] = self.Q[0] + Q_3 * dt
        self.Q[1] = self.Q[1] + Q_4 * dt
        self.Q[2] = self.Q[2] + A3_ * dt + Fc * dt
        self.Q[3] = self.Q[3] + A4_ * dt
        self.Q[4] = self.Q[4] + Q_6 * dt
        self.Q[5] = self.Q[5] - (eta * omega0 * Q_6 + omega0 ** 2 * Q_5) * dt + G0 * omega0 ** 2 * self.base[self.basecount]
        self.acc = Fc - eta1 * omega1 * self.Q[2] - omega1 ** 2 * self.Q[0] * (
                    1 - alpha * d / ((d - self.Q[1]) ** 2 + (self.Q[0]) ** 2) ** 0.5)
        # print(acc)
        # global basecount
        # print(basecount)
        self.acc_end[self.basecount] = self.acc
        self.BASE[self.basecount] = self.Q[4] * V
        self.basecount = self.basecount + 1
        self.ACC[0:self.flength - 1] = self.ACC[1:self.flength]
        self.ACC[self.flength - 1] = self.acc

        self.state = np.array(self.ACC)
        self.push(self.state)
        self.done = 0

        # 定义reward

        self.reward = -(self.acc[0])**2
...
# %% 测试环境
agent = DDPG(state_dim=300, action_dim=1, max_action=1)
...
while j <= args.episode_length - 1:
    ...
    # 状态，实际从传感器读取
    env.state, reward, done = env.step(x0)
    #print(x0)
    acc1__.append(env.state[299])
...
if args.mode == 'train':
    ...
    for i_episode in range(args.max_episode):
        ...
        while (1):
            ...
            action = (action + np.random.normal(0, args.exploration_noise, size=1)).clip(-1, 1)
            ...
            env.state, reward, done = env.step(action)

            next_state = args.state_scale_factor * np.array(env.state)
            ...
            action1__.append(action)
            acc1c__.append(env.state[299])
            #print(acc1c__)
            num += 1
            ep_reward += reward
            ...
            if num >= args.episode_length:
                ...
                print("Episode:{} Total Reward: {:.1f} Explore:{:.2f}".format(i_episode,ep_reward,args.exploration_noise))
                if i_episode % 1 == 0:
...
path_xls = 'motor0on0415_test_true34.xlsx'
