...
parser.add_argument('--max_episode', default=2000, type=int)  # 回合数
...
parser.add_argument('--state_scale_factor', default=0.5, type=float)  # 状态放大系数（放大之后输入网络） 10
parser.add_argument('--action_scale_factor', default=4, type=float)  # 动作放大系数（2网络输出后放大） 2
...
df1 = pd.read_excel('output.xlsx', sheet_name='Data', header=None)
# df1 = pd.read_excel('motor0on.xlsx',sheet_name='Data', header=None)
ii = 20000
x0_array = df1.values[1 + ii:20001 + ii, 1]
...
class Env():
    ...
    def step(self, x0, x1, x2, x3, x4):
        ...
        # 定义reward
        self.reward = -100*((self.y1)**2 + (self.y2)**2 + (self.y3)**2 + (self.y4)**2)
...
class Critic1(nn.Module):
    ...
    def forward(self, x, u):
        ...
        in1, in2, in3, in4 = x.chunk(4, 1)
        x1 = torch.relu(self.ls1_q2(in1))
        x2 = torch.relu(self.ls1_q2(in2))
        x3 = torch.relu(self.ls1_q2(in3))
        x4 = torch.relu(self.ls1_q2(in4))
        x1 = torch.relu(self.ls2_q2(x1))
        x2 = torch.relu(self.ls2_q2(x2))
        x3 = torch.relu(self.ls2_q2(x3))
        x4 = torch.relu(self.ls2_q2(x4))
        u1 = torch.relu(self.la1_q2(u))
        u1 = torch.relu(self.la2_q2(u1))
        c2 = torch.relu(self.l3_q2(torch.cat([x1, x2, x3, x4, u1], 1)))
        c2 = torch.relu(self.l4_q2(c2))
        c2 = self.l5_q2(c2)
        return c, c2
...
Fs = int(1 / env.tau) #1000
nfft = 2 * Fs #2000
fre = Fs / nfft * (np.array(range(0, int(nfft / 2)))) #(0,0.5,,,,,,499.5)
...
if args.mode == 'train':
    ...
    ep_reward__ = []
    plt.figure(figsize=(16, 12))
    for i_episode in range(args.max_episode):##201
        # 进入新的episode之前, 清空列表
        acc1c__ = []
        acc2c__ = []
        acc3c__ = []
        ...
        zero_pit_fill = args.episode_length - 1#10000-1
        while (1):
            ...
            if zero_pit_fill == args.episode_length - 1:  # no zeros, begin to feed to buffer#10000
                agent.replay_buffer.push((state, next_state, action, reward, done))

            state = next_state

            if zero_pit_fill < args.episode_length - 1:#10000
                zero_pit_fill += 1
                continue
            ...
            if num >= args.episode_length:#10000
                ...
                if i_episode % 1 == 0:
                    ...
                    # plt.plot(fre[0:200, ], P1[0:200, ], "r--", label="acc1", linewidth=3)
                    # plt.plot(fre[0:200, ], P1c[0:200, ], "b-", label="acc1 Controlled", linewidth=3)
                    # plt.xlabel('Frequency(Hz)')
                    # plt.ylabel('FFT')
                    # plt.legend()
                    #
                    # plt.subplot(446)
                    # plt.plot(fre[0:200, ], P2[0:200, ], "r--", label="acc2", linewidth=3)
                    # plt.plot(fre[0:200, ], P2c[0:200, ], "b-", label="acc2 Controlled", linewidth=3)
                    # plt.xlabel('Frequency(Hz)')
                    # plt.ylabel('FFT')
                    # plt.legend()
                    #
                    # plt.subplot(447)
                    # plt.plot(fre[0:200, ], P3[0:200, ], "r--", label="acc3", linewidth=3)
                    # plt.plot(fre[0:200, ], P3c[0:200, ], "b-", label="acc3 Controlled", linewidth=3)
                    # plt.xlabel('Frequency(Hz)')
                    # plt.ylabel('FFT')
                    # plt.legend()
                    #
                    # plt.subplot(448)
                    # plt.plot(fre[0:200, ], P4[0:200, ], "r--", label="acc4", linewidth=3)
                    # plt.plot(fre[0:200, ], P4c[0:200, ], "b-", label="acc4 Controlled", linewidth=3)
                    # plt.xlabel('Frequency(Hz)')
                    # plt.ylabel('FFT')
                    # plt.legend()
                    #
                    # plt.subplot(449)
                    # plt.plot(t[0:200], action1__[0:200], 'k-', linewidth=3)
                    # plt.ylabel("Action 1")
                    # plt.xlabel("Time(s)")
                    #
                    # plt.subplot(4, 4, 10)
                    # plt.plot(t[0:200], action2__[0:200], 'k-', linewidth=3)
                    # plt.ylabel("Action 2")
                    # plt.xlabel("Time(s)")
                    #
                    # plt.subplot(4, 4, 11)
                    # plt.plot(t[0:200], action3__[0:200], 'k-', linewidth=3)
                    # plt.ylabel("Action 3")
                    # plt.xlabel("Time(s)")
                    #
                    # plt.subplot(4, 4, 12)
                    # plt.plot(t[0:200], action4__[0:200], 'k-', linewidth=3)
                    # plt.ylabel("Action 4")
                    # plt.xlabel("Time(s)")
                    #
                    # plt.subplot(414)
