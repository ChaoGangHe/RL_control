...
class Actor1(nn.Module):
    ...
    def __init__(self, state_dim, action_dim, max_action):
        ...
        self.conv_1 = nn.Conv1d(in_channels=1,out_channels=8,kernel_size=2)
        self.MaxPool1d_1 = nn.MaxPool1d(2)

        self.conv_2 = nn.Conv1d(in_channels=8, out_channels=16, kernel_size=2)
        self.MaxPool1d_2 = nn.MaxPool1d(2)

        self.conv_3 = nn.Conv1d(in_channels=16, out_channels=32, kernel_size=2)
        self.MaxPool1d_3 = nn.MaxPool1d(4)

        self.conv_4 = nn.Conv1d(in_channels=32, out_channels=64, kernel_size=2)
        self.MaxPool1d_4 = nn.MaxPool1d(4)

        self.l1 = nn.Linear(64*4,128)
        self.l2 = nn.Linear(128,32)
        self.l3 = nn.Linear(32, action_dim)
        self.max_action = max_action

    def forward(self, state):
        in1, in2, in3, in4 = state.chunk(4, 1)#torch.Size([1, 1, 100])
        c1 = self.MaxPool1d_1(torch.tanh(self.conv_1(in1.view(in1.shape[0],1,100))))
        c2 = self.MaxPool1d_1(torch.tanh(self.conv_1(in2.view(in1.shape[0],1,100))))
        c3 = self.MaxPool1d_1(torch.tanh(self.conv_1(in3.view(in1.shape[0],1,100))))
        c4 = self.MaxPool1d_1(torch.tanh(self.conv_1(in4.view(in1.shape[0],1,100))))#[1, 1, 100]->([1, 8, 99])->([1, 8, 49])

        c1 = self.MaxPool1d_2(torch.tanh(self.conv_2(c1)))
        c2 = self.MaxPool1d_2(torch.tanh(self.conv_2(c2)))#[1, 8, 49]->([1, 16, 48])->[1, 16, 24]
        c3 = self.MaxPool1d_2(torch.tanh(self.conv_2(c3)))
        c4 = self.MaxPool1d_2(torch.tanh(self.conv_2(c4)))

        c1 = self.MaxPool1d_3(torch.tanh(self.conv_3(c1)))
        c2 = self.MaxPool1d_3(torch.tanh(self.conv_3(c2)))#[1, 16, 24]-> ([1, 32, 23])->[1, 32, 5]
        c3 = self.MaxPool1d_3(torch.tanh(self.conv_3(c3)))
        c4 = self.MaxPool1d_3(torch.tanh(self.conv_3(c4)))

        c1 = self.MaxPool1d_4(torch.tanh(self.conv_4(c1)))
        c2 = self.MaxPool1d_4(torch.tanh(self.conv_4(c2)))  # [1, 32, 5]-> ([1, 64, 4])->[1, 64, 1]
        c3 = self.MaxPool1d_4(torch.tanh(self.conv_4(c3)))
        c4 = self.MaxPool1d_4(torch.tanh(self.conv_4(c4)))

        c1 = c1.view(c1.shape[0], c1.shape[1])
        c2 = c2.view(c2.shape[0], c2.shape[1])
        c3 = c3.view(c3.shape[0], c3.shape[1])
        c4 = c4.view(c4.shape[0], c4.shape[1])#[1,64]
        x = torch.cat((c1, c2, c3, c4), 1)#[1,64*4]
...
class Critic1(nn.Module):
    def __init__(self, state_dim, action_dim):
        ...
        self.conv_Q1_1 = nn.Conv1d(in_channels=1, out_channels=8, kernel_size=2)
        self.MaxPool1d_Q1_1 = nn.MaxPool1d(2)

        self.conv_Q1_2 = nn.Conv1d(in_channels=8, out_channels=16, kernel_size=2)
        self.MaxPool1d_Q1_2 = nn.MaxPool1d(2)

        self.conv_Q1_3 = nn.Conv1d(in_channels=16, out_channels=32, kernel_size=2)
        self.MaxPool1d_Q1_3 = nn.MaxPool1d(4)

        self.conv_Q1_4 = nn.Conv1d(in_channels=32, out_channels=64, kernel_size=2)
        self.MaxPool1d_Q1_4 = nn.MaxPool1d(4)
        ...
        self.conv_Q2_1 = nn.Conv1d(in_channels=1, out_channels=8, kernel_size=2)
        self.MaxPool1d_Q2_1 = nn.MaxPool1d(2)

        self.conv_Q2_2 = nn.Conv1d(in_channels=8, out_channels=16, kernel_size=2)
        self.MaxPool1d_Q2_2 = nn.MaxPool1d(2)

        self.conv_Q2_3 = nn.Conv1d(in_channels=16, out_channels=32, kernel_size=2)
        self.MaxPool1d_Q2_3 = nn.MaxPool1d(4)

        self.conv_Q2_4 = nn.Conv1d(in_channels=32, out_channels=64, kernel_size=2)
        self.MaxPool1d_Q2_4 = nn.MaxPool1d(4)

        self.l1_q2 = nn.Linear(64 * 4, 128)
        self.l2_q2 = nn.Linear(128, 32)
        self.l3_q2 = nn.Linear(32, 1)


    def forward(self, state, action):
        in1, in2, in3, in4 = state.chunk(4, 1)  # torch.Size([1, 100])
        in1 = torch.cat((in1, action), 1)
        in2 = torch.cat((in2, action), 1)
        in3 = torch.cat((in3, action), 1)
        in4 = torch.cat((in4, action), 1)#[1,100]->[1,104]
        c1 = self.MaxPool1d_Q1_1(torch.tanh(self.conv_Q1_1(in1.view(in1.shape[0], 1, in1.shape[1]))))  # ([1, 1, 104])-->([1, 1, 103])->[1,8,51]
        c2 = self.MaxPool1d_Q1_1(torch.tanh(self.conv_Q1_1(in2.view(in2.shape[0], 1, in2.shape[1]))))
        c3 = self.MaxPool1d_Q1_1(torch.tanh(self.conv_Q1_1(in3.view(in3.shape[0], 1, in3.shape[1]))))
        c4 = self.MaxPool1d_Q1_1(torch.tanh(self.conv_Q1_1(in4.view(in4.shape[0], 1, in4.shape[1]))))

        c1 = self.MaxPool1d_Q1_2(torch.tanh(self.conv_Q1_2(c1)))  # [1,8,51]->[1,16,25]
        c2 = self.MaxPool1d_Q1_2(torch.tanh(self.conv_Q1_2(c2)))
        c3 = self.MaxPool1d_Q1_2(torch.tanh(self.conv_Q1_2(c3)))
        c4 = self.MaxPool1d_Q1_2(torch.tanh(self.conv_Q1_2(c4)))

        c1 = self.MaxPool1d_Q1_3(torch.tanh(self.conv_Q1_3(c1)))  # [1,16,25]->[1,32,6]
        c2 = self.MaxPool1d_Q1_3(torch.tanh(self.conv_Q1_3(c2)))
        c3 = self.MaxPool1d_Q1_3(torch.tanh(self.conv_Q1_3(c3)))
        c4 = self.MaxPool1d_Q1_3(torch.tanh(self.conv_Q1_3(c4)))

        c1 = self.MaxPool1d_Q1_4(torch.tanh(self.conv_Q1_4(c1)))  # [1,32,6]->[1,32,6]
        c2 = self.MaxPool1d_Q1_4(torch.tanh(self.conv_Q1_4(c2)))
        c3 = self.MaxPool1d_Q1_4(torch.tanh(self.conv_Q1_4(c3)))
        c4 = self.MaxPool1d_Q1_4(torch.tanh(self.conv_Q1_4(c4)))
        ...
        in3 = torch.cat((in3, action), 1)
        in4 = torch.cat((in4, action), 1)  # [1,100]->[1,104]
        c1 = self.MaxPool1d_Q2_1(torch.tanh(self.conv_Q2_1(in1.view(in1.shape[0], 1, in1.shape[1]))))  # ([1, 1, 104])-->([1, 1, 103])->[1,8,51]
        c2 = self.MaxPool1d_Q2_1(torch.tanh(self.conv_Q2_1(in2.view(in2.shape[0], 1, in2.shape[1]))))
        c3 = self.MaxPool1d_Q2_1(torch.tanh(self.conv_Q2_1(in3.view(in3.shape[0], 1, in3.shape[1]))))
        c4 = self.MaxPool1d_Q2_1(torch.tanh(self.conv_Q2_1(in4.view(in4.shape[0], 1, in4.shape[1]))))

        c1 = self.MaxPool1d_Q2_2(torch.tanh(self.conv_Q2_2(c1)))  # [1,8,51]->[1,16,25]
        c2 = self.MaxPool1d_Q2_2(torch.tanh(self.conv_Q2_2(c2)))
        c3 = self.MaxPool1d_Q2_2(torch.tanh(self.conv_Q2_2(c3)))
        c4 = self.MaxPool1d_Q2_2(torch.tanh(self.conv_Q2_2(c4)))

        c1 = self.MaxPool1d_Q2_3(torch.tanh(self.conv_Q2_3(c1)))  # [1,16,25]->[1,32,6]
        c2 = self.MaxPool1d_Q2_3(torch.tanh(self.conv_Q2_3(c2)))
        c3 = self.MaxPool1d_Q2_3(torch.tanh(self.conv_Q2_3(c3)))
        c4 = self.MaxPool1d_Q2_3(torch.tanh(self.conv_Q2_3(c4)))

        c1 = self.MaxPool1d_Q2_4(torch.tanh(self.conv_Q2_4(c1)))  # [1,32,6]->[1,32,6]
        c2 = self.MaxPool1d_Q2_4(torch.tanh(self.conv_Q2_4(c2)))
        c3 = self.MaxPool1d_Q2_4(torch.tanh(self.conv_Q2_4(c3)))
        c4 = self.MaxPool1d_Q2_4(torch.tanh(self.conv_Q2_4(c4)))
        ...
        c4 = c4.view(c4.shape[0], c4.shape[1])  # [1,64]
        x = torch.cat((c1, c2, c3, c4), 1)  # [1,64*4]
        q2 = torch.tanh(self.l1_q2(x))
        q2 = torch.tanh(self.l2_q2(q2))
        q2 = self.l3(q2)
        return q1,q2
    ...
    def Q1(self, state, action):
        ...
        in3 = torch.cat((in3, action), 1)
        in4 = torch.cat((in4, action), 1)  # [1,100]->[1,104]
        c1 = self.MaxPool1d_Q1_1(torch.tanh(
            self.conv_Q1_1(in1.view(in1.shape[0], 1, in1.shape[1]))))  # ([1, 1, 104])-->([1, 1, 103])->[1,8,51]
        c2 = self.MaxPool1d_Q1_1(torch.tanh(self.conv_Q1_1(in2.view(in2.shape[0], 1, in2.shape[1]))))
        c3 = self.MaxPool1d_Q1_1(torch.tanh(self.conv_Q1_1(in3.view(in3.shape[0], 1, in3.shape[1]))))
        c4 = self.MaxPool1d_Q1_1(torch.tanh(self.conv_Q1_1(in4.view(in4.shape[0], 1, in4.shape[1]))))
