...
class Actor1(nn.Module):
    ...
    def __init__(self, state_dim, action_dim, max_action):
        ...
        self.lstm1 = nn.LSTM(input_size=400, hidden_size=128, num_layers=1, batch_first=True,
                             dropout=0.5)
    ...
    def forward(self, x):
      ...
      #  print(q1.shape)#torch.Size([1, 1, 400])
        q1 = self.conv(q1)
      #  print(q1.shape)#torch.Size([1, 256, 25])
        q1 = q1.permute(0, 2, 1)  # 交换tensor的维度
       # print(q1.shape)#torch.Size([1, 25, 256])
        # LSTM
        q1, _ = self.lstm1(q1)
       # print(q1.shape)#torch.Size([1, 25, 128])
        q1, _ = self.lstm2(q1)
       # print(q1.shape)#torch.Size([1, 25, 128])
        q1 = self.dropout(q1[:, -1, :])  # 取最后一个时间步的输出
      # print(q1.shape)#torch.Size([1, 128])
        # 全连接层
        q1 = self.fc1(q1)
       # print(q1.shape)#torch.Size([1, 32])
        q1 = torch.tanh(q1)
        q1 = self.fc2(q1)#torch.Size([1, 4])
       # print(q1.shape)
        return self.max_action * q1
...
class Critic1(nn.Module):
    def __init__(self, state_dim, action_dim):
        ...
        self.lstm1 = nn.LSTM(input_size=404, hidden_size=128, num_layers=1, batch_first=True,
                             dropout=0.5)
        self.lstm2 = nn.LSTM(input_size=128, hidden_size=128, num_layers=2, batch_first=True,
                             dropout=0.5)
        ...
        self.lstm1_2 = nn.LSTM(input_size=404, hidden_size=128, num_layers=1, batch_first=True,
                             dropout=0.5)
        self.lstm2_2 = nn.LSTM(input_size=128, hidden_size=128, num_layers=2, batch_first=True,
                             dropout=0.5)

        self.fc1_2 = nn.Linear(128, 32)
        self.fc2_2 = nn.Linear(32, 1)
    ...
    def forward(self, x, u):
        ...
        q2 = self.conv2(q2)
        q2 = q2.permute(0, 2, 1)  # 交换tensor的维度
        # LSTM
        q2, _ = self.lstm1_2(q2)
        q2, _ = self.lstm2_2(q2)
        q2 = self.dropout_2(q2[:, -1, :])  # 取最后一个时间步的输出
        # 全连接层
        q2 = self.fc1_2(q2)
        q2 = torch.tanh(q2)
        q2 = self.fc2_2(q2)
...
path_xls = 'motor0on0415_test_true10.xlsx'
