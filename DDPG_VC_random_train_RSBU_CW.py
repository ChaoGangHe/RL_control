...
#
# class Inceptopnv2(nn.Module):
#     def __init__(self,in_channels=256,ch1=64,ch3_reduce=48,ch3=64,ch5_reduce=64,ch5=96,pool_pro=32):#
#         super(Inceptopnv2, self).__init__()
#         self.branch1x1 = torch.nn.Sequential(
#             torch.nn.Conv1d(in_channels,ch1,kernel_size=1),#[in,64]
#            # torch.nn.BatchNorm1d(ch1,eps=0.0001),
#             torch.nn.Tanh()
#         )
#         self.branch5x5 = torch.nn.Sequential(
#             torch.nn.Conv1d(in_channels,ch3_reduce,kernel_size=1),#[in,48]
#             #torch.nn.BatchNorm1d(ch3_reduce, eps=0.0001),
#             #torch.nn.Tanh(),
#             torch.nn.Conv1d(ch3_reduce,ch3,kernel_size=5,padding=2),#[48,64]
#             #torch.nn.BatchNorm1d(ch3, eps=0.0001),
#             #torch.nn.Tanh(),
#         )
#         self.branch3x3 = torch.nn.Sequential(
#             torch.nn.Conv1d(in_channels,ch5_reduce,kernel_size=1),#[in,64]
#             #torch.nn.BatchNorm1d(ch5_reduce, eps=0.0001),
#             #torch.nn.Tanh(),
#             torch.nn.Conv1d(ch5_reduce,ch5,kernel_size=3,padding=1),#[64,96]
#             #torch.nn.BatchNorm1d(ch5, eps=0.0001),
#             #torch.nn.Tanh(),
#             torch.nn.Conv1d(ch5, ch5, kernel_size=3, padding=1),#[96,96]
#             #torch.nn.BatchNorm1d(ch5, eps=0.0001),
#             #torch.nn.Tanh(),
#         )
#         self.branch_pool = torch.nn.Sequential(
#             torch.nn.AvgPool1d(kernel_size=3,stride=1,padding=1),
#             torch.nn.Conv1d(in_channels,pool_pro,kernel_size=1),#64+64+96+32
#             #torch.nn.BatchNorm1d(pool_pro, eps=0.0001),
#             #torch.nn.Tanh(),
#         )
#     def forward(self,x):
#         return torch.cat([self.branch1x1(x),self.branch5x5(x),self.branch3x3(x),self.branch_pool(x)],1)
#
...
#             torch.nn.Conv1d(4, 8, kernel_size=7, stride=2, padding=3),
#             torch.nn.MaxPool1d(3, 2, padding=1),
#             torch.nn.Conv1d(8, 8, kernel_size=1, stride=1),
#             torch.nn.Conv1d(8, 32,kernel_size=3,stride=1, padding=1),
#             torch.nn.MaxPool1d(3, 2, padding=1),
#             Composition(32, 8,23, 16, 2, 4, 4),
#             torch.nn.MaxPool1d(3, 2, padding=1),
#             Composition(32, 16, 24, 32, 4, 8, 8),
#             torch.nn.MaxPool1d(5, 3),
#             torch.nn.Conv1d(64, 64, kernel_size=1, stride=1),
#         )
#
...
#         x = self.features(x)
#         #print(x.shape)
#         x = self.Linear_action(x.view(x.shape[0],64))
#         x = self.max_action * x
#         return x
#
#
...
class Actor1(nn.Module):
    def __init__(self, state_dim, action_dim, max_action):
        super(Actor1, self).__init__()
        self.features = torch.nn.Sequential(
            torch.nn.Conv1d(4, 8, kernel_size=11, stride=1, padding=5),
            torch.nn.MaxPool1d(3, 2, padding=1),
    ...
    def forward(self, x):
        ...
        x = self.features(x)
        #print(x.shape)
        x = self.Linear_action(x.view(x.shape[0],128))
        x = self.max_action* x
        return x
...
#         #两层卷积层
#         #不同步长只有第一层卷积层不同
#         self.block1 = nn.Sequential(nn.Conv1d(nin, nout, size, stride, padding=1),
#                                     nn.BatchNorm1d(nout),
#                                     nn.ReLU(inplace=True),
#                                     nn.Conv1d(nout, nout, size, 1, padding=1),
#                                     nn.BatchNorm1d(nout))
#         self.shortcut = shortcut
#         #解决通道数变化以及步长不为1引起的图片大小的变化
#         self.block2 = nn.Sequential(nn.Conv1d(nin, nout, size, stride, 1),
#                                     nn.BatchNorm1d(nout))
...
#             nn.ReLU()
#         )
#         self.d1 = self.make_layer(32,32,3,stride=1,t=2)
#         self.d2 = self.make_layer(32,64,3,stride=2,t=2)
#         self.d3 = self.make_layer(64, 128, 3, stride=2, t=2)
#         self.d4 = self.make_layer(128, 256, 3, stride=2, t=2)
#         self.pool =nn.MaxPool1d(3, 2, padding=1),
#         self.avgp = nn.AvgPool1d(8)
#         self.avgp2 = nn.AvgPool1d(2)
